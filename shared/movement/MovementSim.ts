import type { NavMesh, NavPoint } from '../nav/NavMesh.js';
...
export function beginWalk(
  ...
  const dx = x - state.x;
  const dz = z - state.z;
  if (Math.hypot(dx, dz) <= stopRadius) return true;
...
export function cancelWalk(state: MovementState): void {
  state.path = [];
  state.pathIndex = 0;
  state.moving = false;
  state.vx = 0;
...
function shortcutPath(state: MovementState, nav: NavMesh): void {
  if (state.pathIndex >= state.path.length) return;
  ...
  while (state.pathIndex < state.path.length - 1) {
    ...
    if (!nav.canWalkDirectly(here, next)) break;
    state.pathIndex++;
  }
}
